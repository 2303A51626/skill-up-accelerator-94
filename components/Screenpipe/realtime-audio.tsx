...
export function RealtimeAudio({
  ...
  // Helper function to get OpenAI API key with better error handling
  const getOpenAIApiKey = () => {
    // Try multiple possible paths for the API key
    const apiKey = 
      settings?.screenpipeAppSettings?.openaiApiKey ||
  ...
  const generateSummary = async () => {
    ...
    if (!openaiApiKey) {
      const errorMessage = "OpenAI API key not found. Please add your API key in settings.";
      setError(errorMessage);
    ...
    try {
      ...
      const errorMessage = err instanceof Error 
        ...
        : "An error occurred while generating the summary. Please ensure you have a valid OpenAI API key in your settings.";
      setSummary(errorMessage);
      setError(errorMessage);
    } finally {
      setLoading(false);
  ...
  const generateAudio = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error 
        ...
        : "Error generating audio. Please check your API key.";
      setError(errorMessage);
    } finally {
